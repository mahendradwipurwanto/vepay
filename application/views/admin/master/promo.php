	...
	<div class="row align-items-center">
		<div class="col-sm">
			<h1 class="docs-page-header-title">Master promo
				<button type="button" class="btn btn-primary btn-sm float-end" data-bs-toggle="modal" data-bs-target="#tambah"><i class="bi bi-tag"></i> Tambah data</button>
			</h1>
						...
						<?php if (!empty($promo)) : ?>
							...
							foreach ($promo as $val) : ?>
								<tr>
									<td><?= $no++; ?></td>
									<td>
										<a href="<?= site_url('master/penggunaan-promo/' . $val->id); ?>" class="btn btn-soft-primary btn-sm"><i class="bi bi-receipt"></i></a>
										<button type="button" class="btn btn-soft-info btn-sm" onclick="showMdlPromoEdit(<?= $val->id; ?>)"><i class="bi-pencil-square"></i></button>
										<button type="button" class="btn btn-soft-danger btn-sm" data-bs-toggle="modal" data-bs-target="#delete-<?= $val->id; ?>"><i class="bi-trash"></i></button>
									</td>
									<td><?= $val->kode; ?></td>
									<td>
										<?= $val->nama; ?>
								...
								<?= $val->jenis == 1 ? '' : '%'; ?></td> -->
									...
									<td><span class="badge bg-<?= $val->status == 1 ? 'success' : 'danger'; ?>"><?= $val->status == 1 ? 'Aktif' : 'Tidak aktif'; ?></span>
									</td>
									<td><?= date("d F Y", $val->expired); ?></td>
								</tr>

								<div id="delete-<?= $val->id; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="delete" aria-hidden="true">
									<div class="modal-dialog modal-dialog-centered modal-sm" role="document">
										<div class="modal-content">
											...
											<div class="modal-body">
												<form action="<?= site_url('api/master/deletePromo'); ?>" method="post" class="js-validate need-validate" novalidate>
...
<div id="tambah" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="add" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-lg" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form action="<?= site_url('api/master/savePromo'); ?>" method="post" enctype="multipart/form-data" class="js-validate need-validate" novalidate>
					<div class="row">
						<div class="col-6">
							<div class="mb-3">
					...
					<div class="row">
						<div class="col-7">
							<div class="mb-3">
								...
								<input class="form-control form-control-sm" type="text" name="nama" placeholder="Nama Promo" required>
							</div>
						</div>
						<div class="col-5">
							<div class="mb-3">
					...
					<div class="mb-3">
						<figure>
							<img src="#" id="promotambah-preview" class="img-thumbnail img-fluid" alt="Thumbnail image" onerror="this.onerror=null;this.src='<?= base_url(); ?><?= 'assets/images/placeholder.jpg' ?>';">
						</figure>
						<label for="promotambah-upload" class="form-label">Gambar <small class="text-muted">(optional)</small>:</label>
						<div class="input-group">
							<input type="file" class="form-control form-control-sm imgprev" name="image" accept="image/*, .svg" id="promotambah-upload">
						</div>
					...
					<div class="row">
						<div class="col-6">
							<div class="mb-3">
								...
								<input class="form-control form-control-sm" id="inputKodePromo" onkeypress="return event.charCode >= 48" type="text" name="kode" placeholder="Kode promo" required>
							</div>
						</div>
						<div class="col-6">
							<div class="mb-3">
								...
								<div class="input-group input-group-sm flex-nowrap">
									...
									<input type="number" class="form-control form-controls-sm" id="value_promo" name="value" onkeypress="return isNumberKey(event)" placeholder="Nominal/Nilai Promo" required>
								</div>
						...
						<div class="col-12" style="display:none" id="maksimal-nominal">
							<div class="mb-3">
								...
								<div class="input-group input-group-sm flex-nowrap">
									<span class="input-group-text" id="addon-wrapping"><span id="symbol_promo">Rp</span></span>
									<input type="number" class="form-control form-controls-sm" name="maksimal_promo" onkeypress="return event.charCode >= 48" placeholder="Nominal/Nilai Promo">
								</div>
						...
						<div class="col-6">
							<div class="mb-3">
								...
								<div class="input-group input-group-sm flex-nowrap">
									<input type="number" class="form-control form-controls-sm" name="minimum_transaksi" onkeypress="return event.charCode >= 48" placeholder="Minimal transaksi">
									<span class="input-group-text" id="addon-minimum"><span id="symbol_minimum">x
											transaksi</span></span>
								</div>
								<small class="text-secondary">Minimal jumlah transaksi yang dilakukan oleh pengguna, Set 0 untuk disabled</small>
							</div>
						</div>
						<div class="col-6">
							<div class="mb-3">
								<label for="inputSubject" class="form-label">Publish pada</label>
								<input class="form-control form-control-sm" type="date" name="publish" placeholder="pilih tanggal" required>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-6">
							<div class="mb-3">
								...
								<input class="form-control form-control-sm" type="date" name="expired" placeholder="Berlaku sampai" required>
							</div>
						</div>
						<div class="col-6">
							<div class="mb-3">
								<label for="inputSubject" class="form-label">Batas penggunaan</label>
								<input class="form-control form-control-sm" type="number" min="0" max="9999" onkeypress="return event.charCode >= 48" name="quota" placeholder="∞ Unlimited">
					...
					<div class="row">
						<div class="col-12">
							<div class="mb-3">
								...
								<textarea name="desc" id="inputKeterangan" class="form-control form-control-sm" rows="3" placeholder="Keterangan promo"></textarea>
							</div>
						</div>
					</div>
