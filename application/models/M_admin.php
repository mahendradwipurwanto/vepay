...
class M_admin extends CI_Model
{
    ...
    {
        $this->db->select('m_product.name, SUM(_transaksi_detail.quantity) as total_sales, SUM(tb_transaksi.sub_total) as total_profit')
            ->from('m_product')
    ...
    {
        $this->db->select('tb_auth.email, tb_auth.user_id, tb_user.name, COUNT(tb_transaksi.id) as total_sales')
            ->from('tb_user')
            ->join('tb_auth', 'tb_auth.user_id = tb_user.user_id')
